...
def create_ydl_opts(cookies_file=None):
    ...
    opts = {
        ...
        'extractor_args': {
            'youtube': {
                'po_token': 'web',      # 使用 web 客户端的 PO Token
                'player_client': 'web,ios',  # 同时尝试 web 和 ios 客户端
            }
        },
    }
    ...
    if cookies_file:
        ...
        print(f"🍪 同时使用 Cookies 文件: {cookies_file}")
...
def download_audio(cookies_file=None):
    ...
    print(f"📋 使用 PO Token 模式，获取最近 10 个视频...")
            ...
            for url in video_urls:
                try:
                    ...
                    if "Sign in to confirm" in str(e):
                        print("⚠️ PO Token 未能绕过验证，可能需要 Cookies 辅助")
                    else:
                        print(f"下载失败: {e}")
